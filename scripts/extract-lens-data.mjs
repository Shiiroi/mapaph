...
function run() {
	...
	const dataset = {
		...
		counts: {
			...
			municipalities: municities.filter((m) => m.level === 'Municipality').length
		},
		regions,
		provinces,
		municities
	...
	console.log(`Regions: ${regions.length}, Provinces: ${provinces.length}, Municities: ${municities.length}`);
